...
import { getEntities, reset } from './meeting-room.reducer';
...
export const MeetingRoom = () => {
  ...
  const links = useAppSelector(state => state.meetingRoom.links);
  ...
  return (
    <div>
      ...
      <div className="table-responsive">
        <InfiniteScroll
          dataLength={meetingRoomList ? meetingRoomList.length : 0}
          next={handleLoadMore}
        ...
        >
          {meetingRoomList && meetingRoomList.length > 0 ? (
              ...
              <thead>
                <tr>
                  ...
                  <th className="hand" onClick={sort('requiresApproval')}>
                    Requires Approval <FontAwesomeIcon icon={getSortIconByFieldName('requiresApproval')} />
                  </th>
                  <th />
                </tr>
              </thead>
              <tbody>
                {meetingRoomList.map((meetingRoom, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{meetingRoom.requiresApproval ? 'true' : 'false'}</td>
            ...
            !loading && <div className="alert alert-warning">No Meeting Rooms found</div>
          )}
        </InfiniteScroll>
      </div>
    </div>
